...
As cavemen of computer sciences, we don’t like when our TCP routes are congested, no? Here’s how we deal with it. Grab a coffee.
...
We know 11 RTT till first flood. Earlier, $\sum2^k$ worth of data was sent where $k1=0, k2=11$, so calculating that $2^0 + 2^1.....2^{10}+ 2^{11} = 2047$ segments. 2047*1Kb = 2047 Kbs = 2Mb. Now, 14 Mbs are left. Thus, $dataLeftToSend/aws$ = 14/2 = 7 RTTs. Total, 11+7 = 18 RTTs.
...
## Fairness in TCP control

TCP fairness is to ensure that all applications and users receive a fair share of the network resources. TCP fairness works by regulating the rate of data transmission between two hosts. It works by limiting the rate at which one host can send data to the other. This prevents one host from overloading the other with excessive data, thus ensuring that all applications and users receive a fair share of the network resources.

1. **Token Bucket Algorithm**: This algorithm works by controlling the rate of data transmission between two hosts by assigning a token bucket to each host. The bucket contains a certain number of tokens, which can be used to send data. When a token is used, it is removed from the bucket, and the bucket is refilled at a certain rate. The rate at which the buckets are refilled is determined by the bandwidth allocated to each host.
